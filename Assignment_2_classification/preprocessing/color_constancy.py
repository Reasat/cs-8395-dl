...
class ColorConstancy():
    ...
    def compute_cc(self,src,dst,path_skin=None):
        ...
        if self.thresh_bg is None:
            thresh = img[0, 0, :]
            print('setting threshold for background region, ',thresh)
        else:
            thresh=self.thresh_bg
        mask = self.thresh_img(img,thresh)
        mask = np.repeat(mask[:, :, np.newaxis], 3, axis=2)
        img_thresh = img*mask
        img_tx = self.color_constancy(img_thresh)
        io.imsave(dst,img_tx)
...
if __name__=='__main__':
    ...
    color_constancy = ColorConstancy(verbose=False,thresh_bg=[0,0,0])
    ...
    dir_dst = r'D:\Data\cs-8395-dl\assignment2_data_cc-gw\train'
    ...
    for src in tqdm(flpaths):
        ...
        color_constancy.compute_cc(src, dst)
