...
class ResNet18(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        return x
class ResNet18_ms_3(nn.Module):
    def __init__(self,pretrained, bottleneckFeatures=1):
        super(ResNet18_ms_3, self).__init__()
        self.model_z = ResNet18(pretrained,bottleneckFeatures)
        self.model_x = ResNet18(pretrained, bottleneckFeatures)
        self.model_y = ResNet18(pretrained, bottleneckFeatures)
        self.fc = nn.Linear(in_features=6,out_features=2)
    def forward(self, x,y,z):
        x = self.model_x(x)
        y = self.model_y(y)
        z = self.model_z(z)
        ensemble = torch.cat([x,y,z],dim=1)
...
class DPN92(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(in_features=2688, out_features=2)
    def forward(self, x):
...
class DPN68(nn.Module):
    def __init__(self):
        ...
        model=pretrainedmodels.dpn68(num_classes=1000,pretrained='imagenet')
        self.model_fc_stripped = nn.Sequential(*list(model.children())[:-1])
        self.fc = nn.Linear(in_features=832, out_features=2)
    def forward(self, x):
        x = self.model_fc_stripped(x)
        # print(x.shape)
        x = F.avg_pool2d(x,(x.shape[2],x.shape[3]))
...
class UNet11(nn.Module):
    def __init__(self, num_filters=32, pretrained=False):
        ...
        if pretrained==True:
            print('using pretrained (ImageNet) vgg11 as encoder')
        else: 
            print('using vanilla vgg11 as encoder')
...
class UNet16(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, pretrained=False, is_deconv=False):
        ...
        self.pool = nn.MaxPool2d(2, 2)
        if pretrained==True:
            print('using pretrained (ImageNet) vgg16 as encoder')
        else:
            print('using vanilla vgg16 as encoder')
...
def unet16(pretrained=False, **kwargs):
    ...
    pretrained:
            ...
            True  - encoder is pre-trained with VGG11
    """
    model = UNet16(pretrained=pretrained, **kwargs)
    return model
...
if __name__=='__main__':
    model = DPN68()
    print(model)
    x=torch.rand([4,3,512,89])
    # y = torch.rand([4, 3, 512, 89])
    # z = torch.rand([4, 3,147 , 512])
    # np = torch.rand([4, 3, 256, 256])
    output = model(x)
    print(output.shape)
