...
from plotters import recon_img_mask, plot_outline
...
for i_fold, (ind_train, ind_valid) in  enumerate(kf.split(file_ids)):
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        for i_p, ind in enumerate(idx_train):
            ...
            acc_step = 1 #len(batch_start_range)
            ...
            for i_b, batch_start in tqdm(enumerate(batch_start_range)):
                ...
                if epoch == args.epoch//2 and i_p==0 and i_b==0:
                    ...
                    output_3d_rec = (output_3d_rec>output_3d_rec.max()/2)*1
                    plot_outline(image_3d_rec, mask_3d_rec, output_3d_rec,
                                 range(0,output_3d_rec.shape[0],2))

                loss_d = dice_loss(torch.sigmoid(output), mask_batch) / acc_step
                loss_ce = criterion_ce(output, mask_batch) / acc_step
                loss = args.lossWeight[0] * loss_d + args.lossWeight[1] * loss_ce
                loss.backward()


                if (i_b+1)%acc_step ==0:
                    # print('updating weights at batch {}/{}'.format(i_b+1,len(batch_start_range)))
                    optimizer.step()
                    optimizer.zero_grad()
            ...
            print(
                'train >>> epoch: {}/{}, patient {}/{}, mean_loss {:.4f}, mean_dice_loss: {:.4f}, mean_ce: {:.4f}'.format(
                    epoch + 1,
                    args.epoch,
                    i_p+1,
                    len(file_ids_train),
        ...
        with torch.no_grad():
            for ind in tqdm(idx_valid):
                ...
                acc_step = len(batch_start_range)
                ...
                for i_b, batch_start in tqdm(enumerate(batch_start_range)):
                    ...
                    # output_3d_rec = (output_3d_rec>.5)*1
                    # subplot_outline(image_3d_rec,mask_3d_rec,output_3d_rec,
                    #                 range(0,output_3d_rec.shape[0],2))
                ...
                loss_d = dice_loss(torch.sigmoid(torch.tensor(output_3d)), torch.tensor(mask_3d))
                ...
                dice_coef.append(dice(
                    (torch.sigmoid(torch.tensor(output_3d)).numpy() > 0.5) * 1
                    , mask_3d
                ))
        print(
