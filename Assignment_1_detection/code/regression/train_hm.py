...
parser.add_argument('--beta', help='loss parameter', type=int, required=True)
parser.add_argument('--agg', help='loss aggregation', required=True)
...
for epoch in epoch_range:
    ...
    for i, sample in enumerate(loader_train):
        ...
        # print((sample[1] ==1).sum())
        output = torch.sigmoid(model(img))
        # print(target.max(),output.max())
        loss = fl(target,output,alpha=args.alpha,beta=args.beta, agg=args.agg)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for i, sample in enumerate(loader_valid):
            ...
            target = sample[1].to(device)
            output = torch.sigmoid(model(img))
            loss = fl(target,output,alpha=args.alpha,beta=args.beta)
            # print(loss.item())
            running_loss += loss.item()
            mean_loss = running_loss / (i + 1)
            # if epoch % 10 == 0:
